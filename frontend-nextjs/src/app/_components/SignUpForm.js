...
function SignUpForm() {
  ...
  return (
    <div className="tablet:h-110 tablet:w-120 tablet:rounded-3xl h-100 w-full rounded-2xl bg-white lg:h-120 lg:w-120">
      <div className="relative mx-auto flex h-full w-7/8 flex-col items-center gap-3">
        <li className="absolute top-0 left-0 mt-5">
          <IoClose size={34} onClick={close} />
        </li>
        ...
        <button className="mt-5 h-10 w-full rounded-xl bg-red-500">ورود</button>
      </div>
    </div>
  );
}
